...
class MockLLMClient:
    ...
    def chat_stream(
        ...
        # 6. DATA ANALYSIS SKILL
        elif (
            "data analysis" in system_lower
            or "chart" in system_lower
            or "graph" in system_lower
            or "visuali" in system_lower          # visualise / visualize / visualisation
            or any(k in last_msg.content.lower() for k in [
                "chart", "graph", "plot", "trend", "heatmap",
                "candlestick", "waterfall", "scatter", "pie", "bar chart"
            ])
        ):
            yield from self._handle_data_analysis(messages, last_msg, tools)
            return
...
"""
    ...
    def _handle_data_analysis(
        ...
        """Mock handler for the Data Analysis (Graphs & Charts) skill."""

        # ── If a tool result has come back, render the summary ────────────────
        if last_msg.role == "tool" and last_msg.tool_results:
            result_str = last_msg.tool_results[0].content

            summary = f"""### 📊 Data Analysis Report

The chart has been rendered using the requested visualisation tool.
...
| Data points | — |
| Time range  | — |
| Source      | MCP Chart Tool |

#### Interpretation
- The chart highlights the primary trend in the selected dataset.
- Outliers or inflection points are annotated where detected.
- Cross-series comparisons are normalised to a common baseline.

#### Key Takeaway
The visualisation clearly surfaces the pattern requested. Review the chart above for the full picture.

---
*Generated by SkillForge Data Analysis Skill (Mock LLM Offline Fallback).*
"""
            for chunk in self._split_text(summary):
                yield "chunk", chunk
        ...
        # ── Select the best chart tool from the user's query ─────────────────
        user_query = last_msg.content.lower()

        tool_map = [
            ("chart_time_series",         ["trend", "over time", "history", "line", "area", "time series"]),
            ("chart_bar_comparison",      ["compare", "comparison", "bar", "grouped", "stacked"]),
            ("chart_pie_breakdown",       ["breakdown", "mix", "composition", "pie", "donut", "weight"]),
            ("chart_scatter_correlation", ["correlation", "scatter", "relationship", "vs", "versus"]),
            ("chart_heatmap",             ["heatmap", "heat map", "matrix", "correlation matrix"]),
            ("chart_candlestick_ohlc",    ["candlestick", "ohlc", "price action", "open high low close"]),
            ("chart_waterfall",           ["waterfall", "bridge", "build", "walk"]),
            ("chart_export_png",          ["export", "save", "download", "png"]),
        ]

        tool_name = "chart_time_series"  # sensible default
        for name, keywords in tool_map:
            if any(k in user_query for k in keywords):
                tool_name = name
                break

        # Honour only tools that exist in the attached schema
        tool_names: list[str] = []
        if tools:
            for t in tools:
                tool_names.append(
                    t["function"]["name"] if t.get("type") == "function" else t.get("name", "")
                )
        if tool_names and tool_name not in tool_names:
            tool_name = tool_names[0]

        # Extract an optional ticker / dataset label from the message
        ticker = "DATA"
        ticker_match = re.search(r"\b([A-Z]{2,5})\b", last_msg.content)
        if ticker_match:
        ...
        tool_call = ToolCall(
            id="call_chart_1",
            name=tool_name,
            arguments={"ticker": ticker, "chart_type": tool_name},
        )
        yield "done", LLMResponse(content="", tool_calls=[tool_call], stop_reason="tool_use")
